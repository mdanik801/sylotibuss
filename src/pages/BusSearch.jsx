...
export default function BusSearch() {
   // Memoize buses array so it's stable between renders
   const buses = useMemo(
   ...
   // All stops, sorted and unique
   const allStops = useMemo(() => {
      return [...new Set(buses.flatMap((bus) => bus.route))].sort();
   }, [buses]);

   // Compute available "to" stops depending on selected "from"
   ...
   return (
      <div className="relative min-h-screen py-10 px-4">
         ...
         <div className="bg-white/90 p-6 rounded-xl shadow-lg max-w-4xl mx-auto relative">
            ...
            <div>
               <h3 className="text-xl font-semibold text-green-800 mb-4">Available Buses:</h3>
               {results.length > 0 ? (
                  <div className="space-y-4">
                     {results.map((bus) => (
                        <div
                           ...
                           <p className="text-sm text-gray-700 flex items-center gap-2">
                              <FaClock className="text-green-600" /> Departure Time:{" "}
                              {bus.start_time}
                           </p>
                           <p className="text-sm text-gray-700 flex items-center gap-2">
